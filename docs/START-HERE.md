...
## Read these 4 LIVE docs (in order)
...
2. **FLOW-SCRIPT.md** — paste-ready screenplay for Flow's Script tab. (Gemini already split it into Assets once; that project is being abandoned — see below.)
3. **FLOW-STYLE-LOCK-v3.md** — THE style. Grounded simple CARTOON characters (NOT anime, smaller simple eyes) + cinematic painterly backgrounds. Reference: the Church short "A Shower of Heavenly Blessings." The shared style block to paste into every character.
4. **CHARACTERS.md** — the 5 characters' bible (Doug, The Man, The Boy, Mateo, Giving Machines). Note: hazel-eye match between The Man & The Boy is the engine of the film.

## SUPERSEDED — do NOT follow
- `FLOW-STYLE-LOCK-v2.md` (big anime eyes — wrong)
- `STYLE-CANON.md` character look (cel/Ghibli — replaced by v3; only its two-layer *principle* still holds)
...
- **Starting the Flow project OVER.** Prior Flow project drifted (three eye styles, anime creep). New blank "Untitled Story" project exists. Clean slate is intentional.
- Style decision LOCKED: grounded cartoon people + cinematic worlds (v3).
